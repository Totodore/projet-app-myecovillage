...
namespace Project\Models {
	...
	abstract class BaseModel
	{
		...
		public function save(): BaseModel
		{
			...
			$values = array_map(function ($property) { return $this->{$property}; }, array_values($propsToModify));

			if (isset($this->m_id) && static::exists($this->m_id)) {
				$sqlPropsArg = implode(', ', array_map(function ($column) { return $column . ' = ?'; }, array_keys($propsToModify)));
				$pdo->prepare('UPDATE ' . $this->tableName . ' SET ' . $sqlPropsArg . ' WHERE id = ?')->execute(array_merge($values, [$this->m_id]));
			} else {
				$sqlColumnsArgs = implode(', ', array_map(function ($column) { return $column === "id" ? 'NULL' : '?'; }, array_keys($this->properties)));
